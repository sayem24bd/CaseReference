...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const VISITOR_PROXY = "https://casereference.onrender.com/api/visitor";
  
  // ----------------------------
  // Security Sanity Check
  // ----------------------------
  // Ensure DOMPurify exists — mandatory
  if (typeof window.DOMPurify === "undefined") {
    console.error("DOMPurify not found — refusing to render untrusted HTML.");
    // show prominent user-visible message
    const c = document.getElementById("count");
    if (c) c.innerText = "Security: sanitizer missing. অনুগ্রহ করে পেজ রিফ্রেশ করুন।";
  }
  ...
  const renderWithHTML = (htmlText = "") => {
    ...
    if (typeof DOMPurify !== 'undefined') {
      // Strict allowlist: basic formatting tags only
      const clean = DOMPurify.sanitize(s, {
        ALLOWED_TAGS: ['b','i','strong','em','u','p','br','ul','ol','li','a','span'],
      ...
      // As fallback, escape everything (no HTML)
      div.textContent = s;
      return div;
    }
  };
  ...
  function renderWithPagination(list, containerEl, keyword = "") {
    ...
    if ((currentPage * PAGE_SIZE) < list.length) {
      ...
      loadMoreBtn.onclick = () => {
        const pos = loadMoreBtn.offsetTop || 0;
        currentPage++;
  ...
  // ----------------------------
  // ----------------------------
// Fetch data from Firebase RTDB (সুরক্ষিত)
// ----------------------------
const fetchDataJson = async () => {
  try {
    // ১. ডেটাবেসের রুট (/) থেকে ডেটা আনুন
    // firebase.database().ref("/") মানে হলো ডেটাবেসের মূল থেকে শুরু
    ...
    if (Array.isArray(data)) {
      ...
      arr = Object.keys(data).map(k => {
        const item = data[k] || {};
        item.id = item.id ? Number(item.id) : parseInt(k, 10);
        return item;
      });
    } else {
      return [];
    ...
    const validated = arr.filter(it =>
      it && typeof it === "object" &&
      String(it.question || "").trim() &&
      String(it.answer || "").trim()
    );
    return validated;
  } catch (err) {
    console.error("Firebase fetch failed:", err);
    // Firebase SDK-এর নিজস্ব অফলাইন ক্যাশিং আছে, 
    // তবে PWA-এর init() ফাংশনটি এই এরর ধরবে এবং ইউজারকে জানাবে।
    throw new Error("Could not load data from Firebase: " + err.message);
  }
};
